...
const MiniEditor = ({ box, updateBox, onFocus }: { box: CanvasBox, updateBox: any, onFocus: any }) => {
  const editor = useEditor({
    extensions: [
      ...
      TaskList, TaskItem, Subscript, Superscript, Strike, CodeBlock, FontSize, LineHeight, MarkdownPaste
    ],
    content: box.content,
...
const Paper = ({ 
  ...
  const handleCanvasDrop = (e: React.DragEvent) => {
    ...
    else if (dragType === 'horizontalRule') initContent = '<hr>';
      ...
      >
        ...
        >
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-[5]">
           ...
           {/* Metadata Header */}
           <div className="absolute top-0 left-0 right-0 flex items-center justify-between border-b-2 border-slate-900 px-[var(--paper-padding-x)] z-10 no-print" style={{ height: 'var(--header-height)' }}>
             <div className="flex flex-col">
               <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{disciplina || 'Disciplina'}</span>
               <div className="flex items-center gap-2 mt-1">
                  <span className="font-bold text-slate-500 uppercase text-[11px]">{assunto || 'Assunto'}</span>
                  <span className="text-slate-300">|</span>
                  <span className="font-black text-slate-800 text-xl uppercase tracking-tight">{titulo || 'Título'}</span>
                  {subtitulo && (
                    <>
                      <span className="text-slate-300">|</span>
                      <span className="font-medium text-slate-400 uppercase text-[11px]">{subtitulo}</span>
                    </>
                  )}
               </div>
             </div>
...
const Editor: React.FC<EditorProps> = ({
  pages, onAddPage, onRemovePage, onUpdatePage, onSave, onPrint,
